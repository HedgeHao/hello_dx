...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
  ...
  m_modelSkeleton->Initialize(m_Direct3D->GetDevice(),
                              m_Direct3D->GetDeviceContext());

  m_modelPointsCloud = new ModelPointCloud();
  m_modelPointsCloud->Initialize(m_Direct3D->GetDevice(),
                                 m_Direct3D->GetDeviceContext());

  m_modelTest = new ModelTest();
...
bool GraphicsClass::Render() {
  ...
  m_modelTest->update();
  m_modelTest->Render(m_Direct3D->GetDeviceContext());
  result = m_ColorShader->Render(m_Direct3D->GetDeviceContext(),
